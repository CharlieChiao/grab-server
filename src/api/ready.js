...
router.get("/venues", (req, res) => res.json({ ok: true, venues: listVenues().map((venue) => ({ ...venue, payments: getVenue(venue.id)?.payments || null, supportsBookings: typeof getVenue(venue.id)?.listMyBookings === "function" })) }));
