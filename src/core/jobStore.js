...
export function createJob({ userId, createdByUserId = userId, delegationId = null, venueId, target, fireAt }) {
  ...
  db.prepare("INSERT INTO jobs(id,user_id,created_by_user_id,delegation_id,venue_id,target_json,fire_at,status,result_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)")
    .run(id, userId, createdByUserId, delegationId, venueId, JSON.stringify(target), fireAt || null, "pending", null, now, now);
...
export function archiveJob(id) {
  ...
  try {
    db.prepare("INSERT INTO job_history(id,user_id,created_by_user_id,delegation_id,venue_id,target_json,fire_at,status,result_json,created_at,updated_at,archived_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)")
      .run(row.id, row.user_id, row.created_by_user_id, row.delegation_id, row.venue_id, row.target_json, row.fire_at, row.status, row.result_json, row.created_at, row.updated_at, archivedAt);
